...
class Ackley(ObjectiveFn):
    ...
    def __init__(
        ...
        super().__init__(
            dim=dim, bounds=bounds, optimizers=optimizers, optimal_value=0.0, noise_std=noise_std, negate=negate
        )
        self.a = a
        self.b = b
        self.c = c
...
class Rastrigin(ObjectiveFn):
    ...
    def evaluate(self, X: torch.Tensor) -> torch.Tensor:
        return 10.0 * self.dim + torch.sum(
            X**2 - 10.0 * torch.cos(2.0 * math.pi * X),
            dim=-1,
        )
...
class Levy(ObjectiveFn):
    ...
    def evaluate(self, X: torch.Tensor) -> torch.Tensor:
        ...
        part2 = torch.sum(
            (w[..., :-1] - 1.0) ** 2 * (1.0 + 10.0 * torch.sin(math.pi * w[..., :-1] + 1.0) ** 2),
            dim=-1,
        )
...
class Beale(ObjectiveFn):
    ...
    def __init__(self, noise_std: Optional[float] = None, negate: bool = False, bounds: Optional[torch.Tensor] = None):
        if bounds is None:
            bounds = torch.tensor([[-4.5, 4.5], [-4.5, 4.5]])
        optimizers = torch.tensor([[3.0, 0.5]])
        super().__init__(
            dim=2, bounds=bounds, optimizers=optimizers, optimal_value=0.0, noise_std=noise_std, negate=negate
        )
...
class Branin(ObjectiveFn):
    ...
    def __init__(self, noise_std: Optional[float] = None, negate: bool = False, bounds: Optional[torch.Tensor] = None):
        ...
        super().__init__(
            dim=2, bounds=bounds, optimizers=optimizers, optimal_value=0.397887, noise_std=noise_std, negate=negate
        )
